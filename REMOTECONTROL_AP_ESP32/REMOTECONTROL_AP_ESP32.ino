//https://lastminuteengineers.com/creating-esp32-web-server-arduino-ide/
#include <WiFi.h>
#include <WebServer.h>
#include <WiFiManager.h>  // Include WiFiManager
#include <ESPmDNS.h>       // Include mDNS library

/* Put IP Address details */
IPAddress local_ip(192,168,1,1);
IPAddress gateway(192,168,1,1);
IPAddress subnet(255,255,255,0);

WebServer server(80);

uint8_t LED1pin = 4;
bool LED1status = LOW;

uint8_t LED2pin = 5;
bool LED2status = LOW;

const char* hostName = "esp-remote"; // Custom hostname

void setup() {
  Serial.begin(115200);
  pinMode(LED1pin, OUTPUT);
  pinMode(LED2pin, OUTPUT);

  // Initialize WiFiManager
  WiFiManager wifiManager;

  // If the user has already configured Wi-Fi credentials, this line will try to connect using them.
  // If it fails or there are no saved credentials, it will open the captive portal with the given SSID and password.
  if (!wifiManager.autoConnect("REMOTE-CONTROL-SETUP", "portalPassword")) {
    Serial.println("Failed to connect and hit timeout");
    ESP.restart();
  }

  // If connected, print the IP address
  Serial.println("Connected to Wi-Fi!");
  Serial.print("IP Address: ");
  Serial.println(WiFi.localIP());

  // Start mDNS service
  if (!MDNS.begin("name")) {  // Set your domain name here (e.g., "name.local")
    Serial.println("Error setting up MDNS responder!");
  } else {
    Serial.println("mDNS responder started. You can now access the webpage at http://name.local");
  }

  // Start the web server
  server.on("/", handle_OnConnect);
  server.on("/Switch1on", handle_Switch1on);
  server.on("/Switch1off", handle_Switch1off);
  server.on("/Switch2on", handle_Switch2on);
  server.on("/Switch2off", handle_Switch2off);
  server.onNotFound(handle_NotFound);
  
  server.begin();
  Serial.println("HTTP server started");
}


void loop() {
  server.handleClient();
 // MDNS.update(); // Call MDNS update in the loop

  if(LED1status) {
    digitalWrite(LED1pin, HIGH);
  } else {
    digitalWrite(LED1pin, LOW);
  }

  if(LED2status) {
    digitalWrite(LED2pin, HIGH);
  } else {
    digitalWrite(LED2pin, LOW);
  }
}

void handle_OnConnect() {
  LED1status = LOW;
  LED2status = LOW;
  Serial.println("GPIO4 Status: OFF | GPIO5 Status: OFF");
  server.send(200, "text/html", SendHTML(LED1status, LED2status));
}

void handle_Switch1on() {
  LED1status = HIGH;
  Serial.println("GPIO4 Status: ON");
  server.send(200, "text/html", SendHTML(true, LED2status));
}

void handle_Switch1off() {
  LED1status = LOW;
  Serial.println("GPIO4 Status: OFF");
  server.send(200, "text/html", SendHTML(false, LED2status));
}

void handle_Switch2on() {
  LED2status = HIGH;
  Serial.println("GPIO5 Status: ON");
  server.send(200, "text/html", SendHTML(LED1status, true));
}

void handle_Switch2off() {
  LED2status = LOW;
  Serial.println("GPIO5 Status: OFF");
  server.send(200, "text/html", SendHTML(LED1status, false));
}

void handle_NotFound() {
  server.send(404, "text/plain", "Not found");
}

String SendHTML(uint8_t led1stat, uint8_t led2stat) {
  String ptr = "<!DOCTYPE html> <html>\n";
  ptr += "<head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, user-scalable=no\">\n";
  ptr += "<title>LED Control</title>\n";
  ptr += "<style>html { font-family: Helvetica; display: inline-block; margin: 0px auto; text-align: center;}\n";
  ptr += "body{margin-top: 50px;} h1 {color: #444444;margin: 50px auto 30px;} h3 {color: #444444;margin-bottom: 50px;}\n";
  ptr += ".button {display: block;width: 80px;background-color: #3498db;border: none;color: white;padding: 13px 30px;text-decoration: none;font-size: 25px;margin: 0px auto 35px;cursor: pointer;border-radius: 4px;}\n";
  ptr += ".button-on {background-color: #3498db;}\n";
  ptr += ".button-on:active {background-color: #2980b9;}\n";
  ptr += ".button-off {background-color: #34495e;}\n";
  ptr += ".button-off:active {background-color: #2c3e50;}\n";
  ptr += "p {font-size: 14px;color: #888;margin-bottom: 10px;}\n";
  ptr += "</style>\n";
  ptr += "</head>\n";
  ptr += "<body>\n";
  ptr += "<h1>Samvat Remote Control</h1>\n";
  ptr += "<h3>Power in your hands</h3>\n";

  if (led1stat) {
    ptr += "<p>Switch1 Status: ON</p><a class=\"button button-off\" href=\"/Switch1off\">OFF</a>\n";
  } else {
    ptr += "<p>Switch1 Status: OFF</p><a class=\"button button-on\" href=\"/Switch1on\">ON</a>\n";
  }

  if (led2stat) {
    ptr += "<p>Switch2 Status: ON</p><a class=\"button button-off\" href=\"/Switch2off\">OFF</a>\n";
  } else {
    ptr += "<p>Switch2 Status: OFF</p><a class=\"button button-on\" href=\"/Switch2on\">ON</a>\n";
  }

  ptr += "</body>\n";
  ptr += "</html>\n";
  return ptr;
}
